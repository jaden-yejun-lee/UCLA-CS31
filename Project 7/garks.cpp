...
{
    // if gark has been attacked before, gark dies
    if (m_hp == 1) {
        return true;
    }

    else {
        // gark gets attacked and loses a life
        m_hp = 1;

        // if gark is at edge, gark dies and returns true
        if ((m_row == 1 && dir == UP) || (m_col == m_mesa-> cols() && dir == RIGHT) 
             || (m_row == m_mesa -> rows() && dir == DOWN) || (m_col == 1 && dir == LEFT)) {
            m_hp = 0;
            return true;
        }

        // if it is not at the edge then move gark according to dir and return false
        else {
            switch (dir) {
            case UP:
                m_row--;
...
void Player::moveOrAttack(int dir)
{
    ...
    // if player is able to move to next position
    if (m_mesa->determineNewPosition(x, y, dir)) {
        // if number of garks is at least 1
        if (m_mesa->numGarksAt(x, y) >= 1) {
            // attack gark
            m_mesa->attackGarkAt(x, y, dir);
        }
        else {
...
Mesa::~Mesa()
{
    ...
    // delete remaining dynamically allocated garks
...
int Mesa::numGarksAt(int r, int c) const
{
    ...
    int count = 0;
    // iterate through all garks 
    for (int i = 0; i < m_nGarks; i++) {
        // if gark at i has is at row r and column c, increment count
...
bool Mesa::determineNewPosition(int& r, int& c, int dir) const
{
    ...
    // returns false if object goes past any edge
    if (r == 1 && dir == UP) {
        return false;
    }
    ...
    // set new position of object depending on dir
    switch (dir)
    {
    case UP: 
        r--;
...
void Mesa::display() const
{
  ...
  // Indicate each gark's position                                   
    ...
    for (int i = 0; i < m_nGarks; i++) {
        ...
        // if there is a gark set the char to 'G'
        if (gridChar == '.') {
            gridChar = 'G';
        }
        // if there are 2 garks set gridChar to '2'
        else if (gridChar == 'G') {
            gridChar = '2';
        }
        // keep the char at '9' even if it exceeds 9
        else if (gridChar == '9') {
            gridChar = '9';
        }
        // add 1 to gridChar when it is 2 < gridChar < 9
        else {
            gridChar++;
        }
...
{
    // if the number of garks is equal to MAXGARKS return false
...
bool Mesa::attackGarkAt(int r, int c, int dir)
{
    ...
    // if there is at least one gark
    if (numGarksAt(r, c) >= 1) {
        // iterate through all garks
        for (int i = 0; i < m_nGarks; i++) {
            // if gark at i is at row r, column c
            if (m_garks[i]->row() == r && m_garks[i]->col() == c) {
                //if gark dies, delete gark object & move last gark to gark at i & reduce total number of garks & return true
...
bool Mesa::moveGarks()
{
    for (int k = 0; k < m_nGarks; k++)
    {
        ...
        // if gark is on the same position as player, change player to dead
